...
def console():
    ...
    btn_start = ttk.Button(text="Запустить Джарвис | Start", command=click_button)
    btn_on = ttk.Button(text="Включить скрипт | ON", command=click_button_on)
    btn_of = ttk.Button(text="Выключить скрипт | OFF", command=click_button_of)
    btn_start.pack()
    btn_on.pack()
    btn_of.pack()

    label = ttk.Label(text="ДЛЯ ВЫКЛЮЧЕНИЯ СКАЖИТЕ - Джарвис выключись", background="#FFCDD2", foreground="#B71C1C", padding=1)
